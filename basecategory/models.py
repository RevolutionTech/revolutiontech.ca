...
class Button(models.Model):
    ...
    css_class = models.CharField(
        max_length=15, null=True, blank=True, verbose_name="CSS class"
    )
    ...
    def __str__(self):
        return "{text}: {resource}".format(
            text=self.text, resource=self.local_resource.name or self.external_url
        )
...
class Video(models.Model):
    ...
    def youtube_video_code_and_params(self):
        ...
        video_code, params = (
            video_code_and_params_list[0],
            video_code_and_params_list[1:],
        )
...
class Item(OrderedModel):
    ...
    release_year = models.PositiveSmallIntegerField(
        null=True, blank=True, db_index=True, verbose_name="Year released"
    )
    ...
    def image_all(self):
        image_model_set_name = "{item_type}image_set".format(
            item_type=self._meta.model_name
        )
    ...
    def video_all(self):
        video_model_set_name = "{item_type}video_set".format(
            item_type=self._meta.model_name
        )
        return getattr(self, video_model_set_name).all().order_by("id")

    def media_all(self):
        media_model_set_name = "{item_type}media_set".format(
            item_type=self._meta.model_name
        )
